...
let GeminiService = class GeminiService {
    ...
    async analyzeAndGenerateDNA(answers, imageParts) {
...
Add any brand-specific negative prompts based on the questionnaire answers. Output ONLY the two artifacts with their delimiters. No commentary.`;
    ...
    async refineBrandDNA(currentDNA, currentGuardrails, feedback) {
...
---GUARDRAILS_END---`;
    ...
    async generateTestImages(moodboardImages, guardrailsJson, brandDNA) {
        let negativeTerms = [];
        try {
    ...
    async generateSingleImage(styleContext, subject, moodboardImages, avoidClause) {
        try {
            const textPrompt = `${styleContext}\n\nGenerate a photorealistic photograph of the following:\n${subject}${avoidClause}`;
            const parts = [{ text: textPrompt }, ...moodboardImages];
            ...
            console.error(`  Warning: failed to generate test image — ${err.message}`);
        }
        return null;
    }
    parseDNAResponse(responseText) {
        const dnaMatch = responseText.match(/---BRAND_DNA_START---([\s\S]*?)---BRAND_DNA_END---/);
        const guardrailsMatch = responseText.match(/---GUARDRAILS_START---([\s\S]*?)---GUARDRAILS_END---/);
